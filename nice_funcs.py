...
#     #rint(f"Token Overview for: {address}")
#     # endpoint for token overview
#     url = f"{BASE_URL}/token_overview?address={address}"

#     acc_url = f"https://dexscreener.com/solana/{address}"
...
#     if response.status_code == 200:
#         overview_data = response.json()['data']
        
#         #Trade voume in the last hour (buy + sell)
#         buy1h = overview_data.get('buy1h',0)
#         sell1h = overview_data.get('sell1h',0)
#         trade1h = buy1h + sell1h
#         # print(f"Trade 1h: {trade1h}")
...
#         result['buyPercentage'] = buy_percentage
#         result['sellPercentage'] = sell_percentage

#         result['minimum_trades_met'] = True if total_trades >= MIN_TRADES else False
#         result['sell_condition_over'] = True if sell_percentage < MAX_SELL_PERCENTAGE else False

#         # Extract 'priceChangeXhrs'
#         price_changes = {k : v for k, v in overview_data.items() if 'priceChange' in k}
#         # print("Price changes (X) hrs:")
#         # pretty_print_json(price_changes)
#         result['priceChangeXhrs'] = price_changes

#         # Check for rug pull
#         rug_pull = any(value < -80 for key, value in price_changes.items() if value is not None)
#         result['rug_pull'] = rug_pull
#         #if rug_pull:
#             #print("Warning: Price change percentage over -80%, potentional rug pull")

#         # Extract unique wallent count in the last 2 hours
#         unique_wallet1hr = overview_data.get('uniqueWallet1hr', 0)
#         # print(f"Unique wallet 1hr: {unique_wallet1hr}")
#         result['uniqueWallet1hr'] = unique_wallet1hr

#         # Extract volume 24 hr, watches, views 24hr and liquidity
...
#         # print(f"Volume 24h USD: {v24USD}")
#         # print(f"Watch: {watch}")
#         # print(f"Views 24h: {view24h}")
#         # print(f"Liquidity: {liquidity}")

#         # Adding these to the result dic
...
#         # Get the descption and extract specific links
#         dis = overview_data.get('extensions', {})
#         if dis is not None:
...
#             elif 'twitter.com' in url:
#                 links.append({'twiiter': url})
#             elif 'youtube' not in url: 
#                 # Any other url is assumed to be the website
#                 links.append({'website': url})
        
#         #print("Extracted Links")
#         #pretty_print_json(links)
